...
//        String token = FirebaseInstanceId.getInstance().getToken();
        appPrefes.SaveData(Constants.DEVICE_TOKEN, "1234567890ABCGFH");
    }
    ...
    private void callEmailLoginApi() {
        if (cd.isConnectingToInternet()) {
            try {
                ...
                Call<LoginEmailResponseModel> call = apiService.loginEmail(edEmail.getText().toString(), edPassword.getText().toString(), "android", appPrefes.getData(Constants.DEVICE_TOKEN), appPrefes.getData(Constants.DEVICE_TOKEN), "1.0");
    ...
    private void callSocialLoginApi(final String loginType, final String id, final String email, final String name, final String profilePic) {
        if (cd.isConnectingToInternet()) {
            try {
                myProgressDialog.setProgress(false);
                ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
                Call<SocialLoginResponseModel> call = apiService.socialLogin(loginType, id, "android", appPrefes.getData(Constants.DEVICE_TOKEN));
